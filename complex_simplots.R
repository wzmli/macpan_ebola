...
print(gg2 %+% filter(hacksims2, type %in% c("cumIc","cumDc", "newIc", "newDc"))
	+ geom_ribbon(alpha=0.2, aes(ymin=lwr,ymax=upr,fill=effS))
	+ geom_line(data = filter(hacksims,iter==0, type %in% c("cumIc","cumDc","newIc","newDc")),aes(x=newDate,y=value,color=effS))
	+ geom_point(data=filter(hackdat, type %in% c("cumIc","cumDc", "newIc", "newDc")),size=0.5, color="red",aes(x=newDate,y=value))
)
