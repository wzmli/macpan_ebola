...
hackdat <- (dat
	...
	|> mutate(type3 = as.character(type)
		, type3 = ifelse(type3 == "cumDc", "Cumulative deaths",type3)
		, type3 = ifelse(type3 == "cumIc", "Cumulative cases", type3)
		, type3 = ifelse(type3 == "newIc", "Daily new cases", type3)
	)
	|> mutate(type3 = factor(type3, levels=c("Daily new cases", "Cumulative cases","Cumulative deaths")
		)
	)
	|> filter(value>0)
...
hacksims2 <- (hacksims
	|> group_by(newDate,type,effS,type3)
...
#	+ geom_line(data = filter(hacksims,iter==0),aes(x=newDate,y=value,color=effS))
	...
	+ facet_wrap(~type3,scale="free",nrow=1)
	+ theme(legend.position="bottom")
	+ xlab("Date")
	+ geom_vline(aes(xintercept = as.Date("2026-05-28")),linetype = "dotted")
)
...
print(gg2 %+% filter(hacksims2, type3 %in% c("Cumulative cases","Cumulative deaths", "Daily new cases"))
	+ geom_ribbon(alpha=0.2, aes(ymin=lwr,ymax=upr,fill=effS))
