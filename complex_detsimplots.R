...
hacksims2 <- (hacksims
	|> select(newDate,iter,type,value,effS)
	|> group_by(newDate,type,effS)
	|> pivot_wider(names_from=iter, values_from=value)
	|> arrange(type,newDate,effS)
)

print(hacksims2,n=Inf)

gg <- (ggplot(hacksims2,aes(x=newDate))
	+ geom_line(aes(y=det,group=effS,color=effS))
...
print(gg %+% filter(hacksims2, type %in% c("cumDs","cumDc", "cumIs", "cumIc"))
